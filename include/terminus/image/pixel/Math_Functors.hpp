...
struct Arg_Real_Functor : Unary_Return_Template_Type<Make_Real>
{
    ...
    struct ArgImagFunctor : UnaryReturnTemplateType<MakeReal> {
      template <class ValT>
      ValT operator()( ValT const& ) const {
        return ValT();
      }
...
class Std_Dev_Sliding_Functor
{
    ...
    private:
        ...
        double m_mean { 0 }; ///< Store the mean
        
        double m_squared { 0 }; ///< Store sum of differences squared
}; // End class Std_Dev_Sliding_Functor

} // End of tmns::image::pix::math namespace