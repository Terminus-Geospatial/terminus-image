...
ImageResult<Image_Memory<PixelT>> read_image( const std::filesystem::path&        pathname,
                                              const Memory_Driver_Manager::ptr_t  driver_manager = Memory_Driver_Manager::create_defaults() )
{
    tmns::log::info( "Loading image: ", pathname.string() );

    // Create an image resource for the data
    auto driver_res = driver_manager->pick_driver( pathname );

    if( driver_res.has_error() )
    {
        return outcome::fail( driver_res.error().code() );
    }

    return read_image_from_resource( image_resource );
}
